...
1. ~~**DBHandler cache-hit case-sensitivity bug**~~ — **FIXED.** (Task 7, `tests/test_db_handler.py`): `exists()` compared against the literal `"Not Found"` but the app stores `"Not found"` (`NOT_FOUND_ERROR`), so the ratio-based cache-miss check never fired. Fixed by comparing against `NOT_FOUND_ERROR` instead of the hardcoded literal. `test_mostly_not_found_row_reads_as_not_existing` now asserts the correct (cache-miss) outcome.
2. ~~**DBHandler malicious_score/total_scans/tags column misalignment**~~ — **FIXED.** for `ips`/`domains` (Task 7, `tests/test_db_handler.py`): `populate_scores`/`populate_tags` used fixed tuple offsets (`report[2]`/`report[3]`/`report[4]`) correct only for the `hashes` table. Fixed by adding a `SCORE_TAG_COLUMNS` map of `(malicious_score, total_scans, tags)` column positions per `value_type`, and having `populate_scores`/`populate_tags` look up the right indices instead of assuming one fixed layout. `test_domain_report_score_and_tag_fields_are_correct` and the new `test_ip_report_score_and_tag_fields_are_correct` now assert correct values.
3. ~~**extract_table_data's per-result header snapshot**~~ — **FIXED.** (Task 11, `tests/test_vt_tools.py`): `vt_tools.py` built each row from a `headers` set mid-accumulation, not the final set, so rows could end up shorter than the final header list when results had heterogeneous keys. Fixed by splitting the single loop into two passes: first accumulate the complete `headers` set over all results, then build every row against that final set. `test_rows_are_fully_populated_with_final_headers` now asserts every row's length matches the final header count and that values align to the correct header by name.
4. ~~**`ValueReader._accumulate_values` is defined twice**~~ — **FIXED.** (found during the final-review fix cycle, `tests/test_read_file.py`): `app/FileHandler/read_file.py`'s `ValueReader` class defined `_accumulate_values` once for the stdin path (~line 252) and again for the file path (~line 334) — Python kept only the second definition, so `read_from_stdin()` silently wrote into `self.dict_values_file` instead of `self.dict_values` and always returned an empty dict, even though it printed "Successfully read values from user input". This meant the CLI's stdin-piping input mode was completely broken. Fixed by renaming the file-path method to `_accumulate_file_values` (its one call site in `_process_file_lines` updated to match); `read_from_stdin()`'s own `_accumulate_values` no longer gets shadowed. `tests/test_read_file.py`'s `test_read_from_stdin_parses_piped_lines` now asserts the correct behavior instead of characterizing the bug.

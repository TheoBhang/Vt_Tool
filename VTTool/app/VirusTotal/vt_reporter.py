...
class VTReporter:
    ...
    def create_object(self, value_type, value, report):
        ...
        if report != "Not found":
            ...
            malicious_score = (int(malicious)/(int(malicious)+int(undetected) + int(suspicious) + int(harmless)))*100
            suspi_score = (int(suspicious)/(int(malicious)+int(undetected) + int(suspicious) + int(harmless)))*100
            safe_score = (int(harmless)/(int(malicious)+int(undetected) + int(suspicious) + int(harmless)))*100
            if value_type == "PUBLIC IPV4":
                object = {
                    "data" : value,
                ...
                object = {
                    ...
                    "suspicious_score": suspi_score,
                    "safe_score": safe_score,
                    'creation_date': str(utc2local(getattr(report, 'creation_date', 'No creation date found'))),
    ...
    def get_report(self, value_type, value):
        ...
        json_report = self.csv_report(value_type, value, report)
